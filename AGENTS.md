...
- You can use shell commands or Go code to run temporary programs, but do not use any other language such as Python or Perl, these are not available.
...
- Follow modern Go, according to the Go version defined in go.mod. Prefer modern constructs when equivalent, for example:
  - use `for i := range 5` rather than `for i := 0; i < 5; i++`.
  - use `new("string")` rather than helper wrappers such as `stringPtr("string")`.
  - no need to pin variables in for loops when using them in goroutines or subtests.
...
- Make the program OS signal aware, so it attempts a graceful shutdown when interrupted. Force quit the program on a second interrupt signal.
